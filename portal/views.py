...
# ✅ Risolve l'errore AttributeError: module 'portal.views' has no attribute 'LoginView'
...
def home(request):
    try:
        if not request.user.is_staff and request.user.employee.must_change_password:
    ...
    if request.method == "POST":
        for f in request.FILES.getlist("files"):
            try:
                ...
                user, _ = User.objects.get_or_create(username=uname, defaults={'first_name':fn,'last_name':ln,'is_active':True})
                if _: user.set_password("cambiala"); user.save()

                emp, _ = Employee.objects.get_or_create(user=user, defaults={'full_name':f"{fn} {ln}",'must_change_password':True})
...
def admin_manage_employees(request):
    ...
    if request.method == "POST":
        ...
        if email:
            ...
            messages.success(request, f"Inviato a {email}")
    q = request.GET.get("q", "")
...
def open_payslip(request, payslip_id):
    if request.user.is_staff: ps = get_object_or_404(Payslip, id=payslip_id)
    else: ps = get_object_or_404(Payslip, id=payslip_id, employee__user=request.user)
