...
if settings.RICONFEZIONAMENTO_ONLINE_ENABLED:
	...
	def _riconfezionamento_user_from_scope(scope):
		...
		return None if session_hash and session_hash != user.get_session_auth_hash() else user
	...
	class RiconfezionamentoAccessMiddleware:
		...
		async def __call__(self, scope, receive, send):
			...
			if scope.get('type') != 'http' or not request_path.startswith('/riconfezionamento'):
			...
			if user is None:
				...
				next_path = f'{request_path}?{query_string}' if query_string else request_path
				return await RedirectResponse(url=f'{reverse("login")}?next={quote(next_path, safe="")}', status_code=302)(scope, receive, send)
			if not has_access:
				return await PlainTextResponse('Riconfezionamento non disponibile per questo account.', status_code=403)(scope, receive, send)
			return await self.app(scope, receive, send)
